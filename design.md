...
| Web 框架 | FastAPI + Uvicorn | 轻量异步 Web 服务，SSE 流式输出 |
...
基于 FastAPI 提供 Web 问答界面，支持浏览器访问。

- **`web.py`**：FastAPI 应用，lifespan 管理组件初始化与销毁
  - `GET /`：返回前端页面
  - `POST /api/query`：流式问答接口（SSE），先推送参考来源，再逐 token 输出回答
  - `POST /api/search`：纯检索接口，返回带分数的 TopK 结果
- **`static/index.html`**：单页前端，支持两种模式
  - 对话模式：流式显示回答，支持 Markdown 渲染（代码块、粗体、行内代码）
  - 检索模式：展示检索结果列表（文件路径、内容摘要、相关度分数）
- CLI 命令 `qa web`：启动 Web 服务，支持 `--host` 和 `--port` 参数
- 依赖新增 `fastapi`、`uvicorn`

**产出：** `notes_qa/web.py`, `notes_qa/static/index.html`

### Phase 9: 对话历史记录

为 Web 端添加对话历史记录功能，支持持久化存储。

- **`history.py`**：对话历史管理模块
  - `ChatHistory` 类：SQLite 数据库管理
  - `conversations` 表：存储对话元信息（ID、标题、创建时间、更新时间）
  - `messages` 表：存储消息内容（角色、内容、参考来源、模式、时间）
  - 支持操作：创建对话、添加消息、获取对话列表、获取对话详情、删除对话、更新标题、搜索消息
- **`web.py`** 更新：
  - 修改 `/api/query` 和 `/api/search` 接口，支持 `conversation_id` 参数
  - 新增历史记录 API 端点：
    - `GET /api/conversations`：获取对话列表
    - `GET /api/conversations/{id}`：获取对话详情及消息
    - `DELETE /api/conversations/{id}`：删除对话
    - `PUT /api/conversations/{id}/title`：更新对话标题
    - `GET /api/search-messages`：搜索消息内容
...
**产出：** `notes_qa/history.py`, 更新 `notes_qa/web.py`, `notes_qa/static/index.html`

### Phase 10: 纯 Agent 检索模式

实现不依赖索引的纯 Agent 检索功能，通过工具调用让 LLM 自主检索笔记。

- **`agent_search.py`**：Agent 检索引擎
  - `AgentSearchEngine` 类：基于 OpenAI 工具调用的 Agent 实现
  - 工具定义：
    - `list_files`：列出笔记目录结构
    - `read_file`：读取单个文件内容
    - `search_keyword`：在文件中搜索关键词
    - `finish`：返回最终答案和参考来源
  - 多轮对话机制：LLM 自主决定使用哪些工具
  - 限制机制：最大轮数限制、最大文件读取数限制
  - 详细日志：显示完整的工具调用过程和返回内容
- **`cli.py`** 更新：
  - 新增 `agent` 子命令：`qa agent "问题"`
  - 选项：`--max-rounds/-r`（最大轮数）、`--max-files/-f`（最大文件数）

**产出：** `notes_qa/agent_search.py`, 更新 `notes_qa/cli.py`
...
5. 运行 `qa agent "某个问题"` 验证纯 Agent 检索功能
6. 运行单元测试确认各模块基本逻辑正确

## 后续扩展

- ~~Web 界面（FastAPI + 前端）~~ ✅ 已完成
- ~~对话历史记录~~ ✅ 已完成
- ~~纯 Agent 检索模式~~ ✅ 已完成
- 增量索引更新（监听文件变化）
- 支持更多文件格式（PDF、docx 等）
- 多笔记库管理
- Web 界面功能增强（主题切换、导出功能等）
